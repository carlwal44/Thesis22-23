...
import win_prediction._init_paths
#import _init_paths
...
def infer():
    ...
    logger.warning("Using Devices: {}".format(str(devices)))
    ...
    logger.warning('Speed: %.3f second per image' % (endt / len(infer_imdbs)))
    logger.warning("Save inference results into %s"%output_path)
