...
def parse_args():
    ...
    path_to_join = os.path.dirname(os.path.dirname(__file__))
    parser = argparse.ArgumentParser(description='window detection')
    # yaml config file
    parser.add_argument('--cfg', help='experiment configure file name', required=False, type=str, default= os.path.join(path_to_join,"experiments/resnet/lr1e-3_x120-90-110_center_b2.yaml"))
    parser.add_argument('--model', help='/path/to/model, requird when resume/test', type=str, default= os.path.join(path_to_join,"model/resnet18_model_latest.pth.tar"))
    ...
    parser.add_argument('--infer', help='/path/to/image/folder', type=str, default= os.path.join(path_to_join,"result"))
