...
def to_xml(windows_list, imdb_list, save_path, saveXML = False, printXML = False):
    ...
    for s_idx in range(len(windows_list)):
        ...
        for i in range(len(winPred)):
            windowImg = get_window_image(winPred[i], img)
            windowBlack, windowColour = getWindowColour(windowImg)
            colourList.append(windowColour)
            
        
        colors_array = np.array(colourList)

        # Calculate z-scores for each color channel
        z_scores = stats.zscore(colors_array, axis=0)

        # Set a threshold for z-score to consider an outlier
        z_score_threshold = 1.5

        # Find indices of non-outlier colors
        non_outlier_indices = np.all(np.abs(z_scores) < z_score_threshold, axis=1)
        

        # Calculate the average color of non-outlier colors
        windowColour = np.average(colors_array[non_outlier_indices], axis=0)
        windo_color_elemenent = ET.Element("window_color")
        image_element.append(windo_color_elemenent)
        R_color = ET.SubElement(windo_color_elemenent, "R")
        R_color.text = str(windowColour[0])
        G_color = ET.SubElement(windo_color_elemenent, "G")
        G_color.text = str(windowColour[1])
        B_color = ET.SubElement(windo_color_elemenent, "B")
        B_color.text = str(windowColour[2])

        #print("Average Color (excluding outliers):", average_color)
...
def set_xml_average(etageElement, etageList, image, model):
    ...
    class_indx = {'class_1': 0, 'class_2': 1, 'class_3': 2, 'class_4': 3, 'class_5': 4}
    ...
    for l in range(len(groups1)):
        ...
        for k in range(len(groups2)):
            ...
            for j in range(len(groups3)):
                ...
                predictionList = []
                for w in range(len(group3)):
                    rectWindow = get_window_image(group3[w], im, True)
                    classOfWindow, predictions = predict_windowTpye(rectWindow, model)
                    predictionList.append(predictions)
                
                array = np.array(predictionList)
                average_array = np.mean(array, axis=0)
                predicted_class_index = np.argmax(average_array)
                
                predicted_class_name = [u for u, v in class_indx.items() if v == predicted_class_index][0]
                ...
                for i in range(len(group3)):
                    ...
                    #classOfWindow, predictions = predict_windowTpye(rectWin, model)
                    windowType.text = predicted_class_name      
...
def removeMiddleOfWindow(image):
    mask = np.zeros(image.shape[:2], dtype="uint8")
    imShape = image.shape
    imHeight = imShape[0]
    imWidth = imShape[1]
    cv2.rectangle(mask,  (int(imWidth*0.09), int(imHeight *0.04)), 
                (int(imWidth*0.91), int(imHeight *0.96)), 255, -1)    
    masked = cv2.bitwise_and(image, image, mask=cv2.bitwise_not(mask))
    return masked

def getWindowColour(window, blur = False):
    if blur:
        window = cv2.GaussianBlur(window, (33,21),0)
    img = cv2.cvtColor(window, cv2.COLOR_BGR2RGB)
    ...
    img_filtered = removeMiddleOfWindow(img_filtered)
